...
// neverAcquireStore simulates a genuinely completed duplicate: SetProcessing
// returns false and IsCompleted returns true, so the gate ACKs and skips.
...
func (n *neverAcquireStore) IsCompleted(_ context.Context, _ string) (bool, error) {
	return true, nil // completed → genuine duplicate
}
func (n *neverAcquireStore) ClearProcessing(_ context.Context, _ string) error { return nil }
func (n *neverAcquireStore) TryReclaimStale(_ context.Context, _, _ string, _ time.Duration) (bool, error) {
	return false, nil // not reached: IsCompleted=true short-circuits
}

var _ ports.IdempotencyStore = (*neverAcquireStore)(nil)

// staleLockStore simulates a stale processing lock: SetProcessing returns
// false, IsCompleted returns false (it's "processing", not "completed"), and
// TryReclaimStale returns true (lock is old enough to reclaim).
type staleLockStore struct{ *stubs.StubIdempotencyStore }

func newStaleLockStore() *staleLockStore {
	return &staleLockStore{StubIdempotencyStore: stubs.NewStubIdempotencyStore()}
}
func (s *staleLockStore) SetProcessing(_ context.Context, _, _ string) (bool, error) {
	return false, nil // lock already exists
}
func (s *staleLockStore) IsCompleted(_ context.Context, _ string) (bool, error) {
	return false, nil // status is "processing", not "completed"
}
func (s *staleLockStore) TryReclaimStale(_ context.Context, _, _ string, _ time.Duration) (bool, error) {
	return true, nil // lock is stale — reclaim succeeds
}

var _ ports.IdempotencyStore = (*staleLockStore)(nil)
...
func (d *countingDLQWriter) SendToDLQ(ctx context.Context, e *domain.DLQEntry) error {
	atomic.AddInt64(&d.calls, 1)
	return d.StubDLQWriter.SendToDLQ(ctx, e)
}
...
func (r *countingRetryScheduler) ScheduleRetry(ctx context.Context, t *domain.EmailTask, d time.Duration) error {
	atomic.AddInt64(&r.calls, 1)
	return r.StubRetryScheduler.ScheduleRetry(ctx, t, d)
}
...
func (t *trackingIdempotencyStore) ClearProcessing(ctx context.Context, id string) error {
	atomic.AddInt64(&t.cleared, 1)
	return t.StubIdempotencyStore.ClearProcessing(ctx, id)
}
func (t *trackingIdempotencyStore) TryReclaimStale(ctx context.Context, id, w string, d time.Duration) (bool, error) {
	return t.StubIdempotencyStore.TryReclaimStale(ctx, id, w, d)
}
...
func TestSupervisor_ProcessesTaskSuccessfully(t *testing.T) {
	...
	sv := newSupervisor(cfg, cons, sender,
		stubs.NewStubIdempotencyStore(), stubs.NewStubRetryScheduler(), stubs.NewStubDLQWriter())
	...
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&sender.calls) == 1 },
		3*time.Second, 10*time.Millisecond, "sender must be called once")
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&cons.acked) == 1 },
...
func TestSupervisor_PanicRecovery(t *testing.T) {
	...
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&cons.nacked) == 1 },
	...
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&sender.calls) >= 2 },
...
func TestSupervisor_IdempotentSkip(t *testing.T) {
	...
	sender := &countingSender{}
	sv := newSupervisor(cfg, cons, sender,
		&neverAcquireStore{}, stubs.NewStubRetryScheduler(), stubs.NewStubDLQWriter())
	...
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&cons.acked) == 1 },
		3*time.Second, 10*time.Millisecond, "completed duplicate must be ACK'd")
	assert.Equal(t, int64(0), atomic.LoadInt64(&sender.calls), "sender must not be called")

	cancel()
	require.NoError(t, <-runDone)
}

// TestSupervisor_StaleLockReclaim verifies that when SetProcessing returns
// acquired=false and IsCompleted=false, TryReclaimStale is called and — if it
// succeeds — the task is processed rather than skipped.
func TestSupervisor_StaleLockReclaim(t *testing.T) {
	cfg := makeTestConfig()
	cons := newChanConsumer(4)
	sender := &countingSender{}
	sv := newSupervisor(cfg, cons, sender,
		newStaleLockStore(), stubs.NewStubRetryScheduler(), stubs.NewStubDLQWriter())
	...
	cons.send(validSupervisorTask("task-stale-01"))

	// Stale lock was reclaimed → sender must be called (task processed, not dropped).
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&sender.calls) == 1 },
		3*time.Second, 10*time.Millisecond, "task must be processed after stale lock reclaim")
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&cons.acked) == 1 },
		3*time.Second, 10*time.Millisecond, "task must be ACK'd after stale reclaim")
...
func TestSupervisor_RetriesOnFailure(t *testing.T) {
	...
	sv := newSupervisor(cfg, cons, sender,
		stubs.NewStubIdempotencyStore(), retryScheduler, stubs.NewStubDLQWriter())
	...
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&retryScheduler.calls) == 1 },
...
func TestSupervisor_SendsToDLQOnMaxAttempts(t *testing.T) {
	...
	dlq := newCountingDLQ()
	sv := newSupervisor(cfg, cons, sender,
		stubs.NewStubIdempotencyStore(), stubs.NewStubRetryScheduler(), dlq)
	...
	cons.send(task)
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&dlq.calls) == 1 },
...
func TestSupervisor_ScheduleRetryFailure_ClearsLock(t *testing.T) {
	...
	sv := newSupervisor(cfg, cons, sender, idempotency, newFailingRetry(), stubs.NewStubDLQWriter())
	...
	assert.Equal(t, int64(1), atomic.LoadInt64(&idempotency.cleared),
		"ClearProcessing must be called before Nack")
...
func TestSupervisor_NonRetryableGoesToDLQ(t *testing.T) {
	...
	retryScheduler := newCountingRetry()
	sv := newSupervisor(cfg, cons, sender,
		stubs.NewStubIdempotencyStore(), retryScheduler, dlq)
	...
	cons.send(task)
	assert.Eventually(t, func() bool { return atomic.LoadInt64(&dlq.calls) == 1 },
		3*time.Second, 10*time.Millisecond, "non-retryable must go to DLQ")
